#!/usr/bin/env python3
"""
Create GG6.zip with Unix path separators for Netlify deployment
"""
import os
import zipfile
...
def create_zip():
    # Remove old ZIP
    zip_path = Path("GG6.zip")
    if zip_path.exists():
        zip_path.unlink()
    
    # Create ZIP with Unix paths
    with zipfile.ZipFile("GG6.zip", "w", zipfile.ZIP_DEFLATED) as zipf:
        gg6_path = Path("GG6")
        
        # Add all files from GG6 folder
        for file_path in gg6_path.rglob("*"):
            if file_path.is_file():
                # Get relative path
                relative_path = file_path.relative_to(gg6_path)
    ...
    print(f"\nGG6.zip created successfully!")
    print(f"Size: {zip_size:.2f} KB")
    
    # Verify critical files
    with zipfile.ZipFile("GG6.zip", "r") as zipf:
        critical_files = [
