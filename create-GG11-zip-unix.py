#!/usr/bin/env python3
"""
Create GG11.zip with Unix path separators for Netlify deployment
"""
import os
import zipfile
...
def create_zip():
    # Remove old ZIP
    zip_path = Path("GG11.zip")
    if zip_path.exists():
        zip_path.unlink()
    
    # Create ZIP with Unix paths
    with zipfile.ZipFile("GG11.zip", "w", zipfile.ZIP_DEFLATED) as zipf:
        gg11_path = Path("GG11")
        
        # Add all files from GG11 folder
        for file_path in gg11_path.rglob("*"):
            if file_path.is_file():
                # Get relative path
                relative_path = file_path.relative_to(gg11_path)
    ...
    print(f"\nGG11.zip created successfully!")
    print(f"Size: {zip_size:.2f} KB")
    
    # Verify critical files
    with zipfile.ZipFile("GG11.zip", "r") as zipf:
        critical_files = [
