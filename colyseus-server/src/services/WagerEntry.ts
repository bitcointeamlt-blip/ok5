...
const RPCS = [...new Set((process.env.RONKE_RPCS || [process.env.RONIN_MAINNET_RPC, process.env.RONKE_RPC_URL, "https://api.roninchain.com/rpc", "https://ronin.drpc.org"].filter(Boolean).join(",")).split(",").map((s) => s.trim()).filter(Boolean))];
