...
const MAX_ARMOR_REGEN_PER_TICK = 2; // frontend: +1 base, +2 if NFT>=1 (capped)
...
export class GameRoom extends Room<GameState> {
  ...
  async onJoin(client: Client, options: any) {
    ...
    this.dmgBySid.set(client.sessionId, clampNum(options?.dmg, 1, 999, 1));
    this.critChanceBySid.set(client.sessionId, clampNum(options?.critChance, 0, 95, 4));
    this.accuracyBySid.set(client.sessionId, clampNum(options?.accuracy, 0, 100, 60));
    this.lastFireAtBySid.set(client.sessionId, { bullet: 0, projectile: 0, mine: 0, spike: 0 });
    player.ready = false;
  ...
  handlePlayerInput(client: Client, message: any) {
    ...
    try {
        ...
        if (typeof sanitizedInput.armor === "number") {
          ...
          if (enforce) {
            ...
            if (nextArmor > curArmor) {
              ...
              const canRegen =
                dArmor > 0 &&
                dArmor <= MAX_ARMOR_REGEN_PER_TICK &&
      ...
      if (t === "hit") {
        ...
        let projType: "bullet" | "projectile" | "mine" | "spike" = "projectile";
        ...
        const fires = this.lastFireAtBySid.get(client.sessionId) || { bullet: 0, projectile: 0, mine: 0, spike: 0 };
        ...
        const isCrit = r01 < (critChance / 100);
  ...
  private applyInputToState(sessionId: string, input: PlayerInputMessage): void {
    ...
    try {
      const now = Date.now();
      const fires = this.lastFireAtBySid.get(sessionId) || { bullet: 0, projectile: 0, mine: 0, spike: 0 };
