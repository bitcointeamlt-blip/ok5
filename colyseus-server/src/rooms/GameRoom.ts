...
export class GameRoom extends Room<GameState> {
  ...
  onJoin(client: Client, options: any) {
    ...
    // IMPORTANT: Clamp to sane PvP bounds. Even if client tries to spoof absurd stats, they won't exceed these caps.
  ...
  onLeave(client: Client, consented: boolean) {
    ...
    if (this.state.gameStarted && !this._matchEnded) {
  ...
  handlePlayerInput(client: Client, message: any) {
    ...
    try {
      ...
      if (t === "hit") {
        ...
        // Clamp damage and prevent negative heal exploits.
        const dRaw = (sanitizedInput as any).damage;
        const d = (typeof dRaw === "number" && Number.isFinite(dRaw)) ? Math.max(0, Math.min(MAX_DAMAGE_PER_HIT, Math.round(dRaw))) : 0;
        (sanitizedInput as any).damage = d;
      }
    } catch {}
    // #region agent log
    try {
    ...
    if (input.type === "arrow" && typeof input.x === "number" && typeof input.y === "number" && typeof input.targetX === "number" && typeof input.targetY === "number") {
      ...
      if (targetId) {
        ...
        if (dist > 0.0001) {
          ...
          // Clamp offensive stats to prevent client spoofing absurd upgrades.
          const dmgRaw = (typeof input.dmg === "number" && Number.isFinite(input.dmg)) ? input.dmg : 1;
          const critRaw = (typeof input.critChance === "number" && Number.isFinite(input.critChance)) ? input.critChance : 4;
          const dmg = Math.max(1, Math.min(50, Math.round(dmgRaw)));
          const critChance = Math.max(0, Math.min(50, Math.round(critRaw)));
  ...
  private applyHit(player: Player, input: PlayerInputMessage): void {
    ...
    const dmg = (typeof input.damage === "number" && Number.isFinite(input.damage))
      ...
      : 0;
    if (!dmg) return;
    const absorbed = Math.min(dmg, player.armor);
