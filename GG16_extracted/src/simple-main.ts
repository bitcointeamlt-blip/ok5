...
type PvPPlayer = {
  ...
  lastCollisionDamageTime: number; // When last collision damage was dealt
};
...
let opponentDmg = 1; // Opponent's damage stat (default to 1, will be updated from arrow launch event)
...
function initializePvPWithColyseus(room: any, mySessionId: string, opponentSessionId: string): void {
  ...
  // Initialize opponent - stats will be synced from network (opponent's own stats)
  const opponent: PvPPlayer = {
    id: opponentId,
    x: isMePlayer1 
    ...
    gravityLocked: true,
    speedTrail: [],
    hp: 100, // Default - will be synced from network with opponent's actual stats
    maxHP: 100, // Default - will be synced from network with opponent's actual stats
    armor: 50, // Default - will be synced from network with opponent's actual stats
    maxArmor: 50, // Default - will be synced from network with opponent's actual stats
    outOfBoundsStartTime: null,
    lastOutOfBoundsDamageTime: 0,
    lastArmorRegen: Date.now(),
    lastCollisionDamageTime: 0,
  ...
  // Send my stats to opponent when initializing (so they know my HP/Armor)
...
function initializePvPWithMatch(match: Match, isPlayer1: boolean): void {
  ...
  // Stats will be synced from network (opponent's own stats, not mine)
  const opponent: PvPPlayer = {
    id: opponentId,
    x: isMePlayer1 
      ? pvpBounds.left + (pvpBounds.right - pvpBounds.left) * 0.75  // RIGHT if I'm p1
...
function handleOpponentInput(input: any): void {
  ...
  if (input.type === 'arrow' && input.x !== undefined && input.y !== undefined) {
    ...
    // Save opponent's damage stat for damage calculation
    if (input.dmg !== undefined) {
      opponentDmg = input.dmg;
    }
    ...
    console.log('Opponent arrow launched', { x: input.x, y: input.y, targetX: input.targetX, targetY: input.targetY, dmg: opponentDmg });
  }
  ...
  if (input.type === 'projectile' && input.x !== undefined && input.y !== undefined) {
    ...
    console.log('Opponent projectile launched', { x: input.x, y: input.y, targetX: input.targetX, targetY: input.targetY, dmg: opponentDmg });
  }
  ...
  // This is opponent's OWN stats, not mine - opponent sends their stats to me
  if (input.type === 'stats' && input.hp !== undefined && input.armor !== undefined) {
    // Update opponent's stats from network (opponent's own stats)
    if (opponentId && pvpPlayers[opponentId]) {
      const opponent = pvpPlayers[opponentId];
      opponent.hp = input.hp; // Opponent's HP
      opponent.armor = input.armor; // Opponent's Armor
      if (input.maxHP !== undefined) opponent.maxHP = input.maxHP; // Opponent's Max HP
      if (input.maxArmor !== undefined) opponent.maxArmor = input.maxArmor; // Opponent's Max Armor
      console.log('Received opponent stats update (opponent\'s own stats)', { 
        hp: input.hp, 
        armor: input.armor,
        maxHP: input.maxHP,
...
// This sends MY OWN stats to opponent, so they know my HP/Armor
  ...
  if (gameMode === 'PvP' && currentMatch) {
    ...
    if (isSyncing) {
      const statsInput = {
        ...
        timestamp: Date.now(),
        hp: hp, // My HP
        armor: armor, // My Armor
        maxHP: maxHP, // My Max HP
        maxArmor: maxArmor // My Max Armor
      };
      ...
      console.log('Sent my stats to opponent', { hp, armor, maxHP, maxArmor });
    }
  }
}
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    for (const playerId in pvpPlayers) {
      ...
      if (myPlayerId && playerId === myPlayerId) {
        ...
        // Draw opponent stats below my stats (if opponent exists)
        if (opponentId && pvpPlayers[opponentId]) {
          const opponent = pvpPlayers[opponentId];
          const opponentStatsX = 20;
          const opponentStatsY = statsY + statsHeight + 10; // Below my stats with 10px gap
          const opponentStatsWidth = 200;
          const opponentStatsHeight = 50;
          
          // Frame background
          ctx.fillStyle = '#ffffff';
          ctx.fillRect(opponentStatsX, opponentStatsY, opponentStatsWidth, opponentStatsHeight);
          
          // Frame border
          ctx.strokeStyle = '#000000';
          ctx.lineWidth = 2;
          ctx.strokeRect(opponentStatsX, opponentStatsY, opponentStatsWidth, opponentStatsHeight);
          
          // Label: "OPPONENT STATS"
          ctx.fillStyle = '#000000';
          ctx.font = 'bold 10px "Press Start 2P"';
          ctx.textAlign = 'left';
          ctx.fillText('OPPONENT STATS', opponentStatsX + 10, opponentStatsY + 15);
          
          // HP and Armor text
          ctx.font = 'bold 12px "Press Start 2P"';
          ctx.fillText(`HP: ${opponent.hp}/${opponent.maxHP}`, opponentStatsX + 10, opponentStatsY + 30);
          ctx.fillText(`ARMOR: ${opponent.armor}/${opponent.maxArmor}`, opponentStatsX + 10, opponentStatsY + 50);
        }
    ...
    if (distance > 0) {
      ...
      if (currentMatch && isSyncing) {
        ...
        const projectileInput = {
          ...
          dmg: dmg, // Send my damage stat so opponent can calculate correct damage
        };
        if (useColyseus) {
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    if ((gameMode === 'PvP' || gameMode === 'Training') && pvpKatanaFlying && opponentId && pvpPlayers[opponentId]) {
      ...
      if (distanceSquared <= hitRadiusSquared) {
        ...
        // Impact! Apply 2x damage (normal) or 3x damage (crit)
        const arrowDamage = isCritHit ? dmg * 3 : dmg * 2; // 2x normal, 3x crit
        ...
        // NOTE: Do NOT send stats update here - opponent will calculate damage themselves
        // when they receive the arrow hit event. Sending opponent's stats here would
        // cause opponent to see their own HP decrease incorrectly.
    ...
    if (gameMode === 'PvP' && opponentArrowFlying && myPlayerId && pvpPlayers[myPlayerId]) {
      ...
      if (distanceSquared <= hitRadiusSquared) {
        const distance = Math.sqrt(distanceSquared);
        // Check for crit hit (using opponent's crit chance - we don't have it, so use our own)
        // Note: In a real implementation, we'd need to sync opponent's critChance too
        let isCritHit = Math.random() < critChance / 100;
        
        // Impact! Apply 2x damage (normal) or 3x damage (crit) using opponent's damage stat
        const arrowDamage = isCritHit ? opponentDmg * 3 : opponentDmg * 2; // 2x normal, 3x crit
        ...
        // Send stats update to opponent (they need to know my HP/Armor)
        sendStatsUpdate(myPlayer.hp, myPlayer.armor, myPlayer.maxHP, myPlayer.maxArmor);
      ...
      if (now - opponentProjectileSpawnTime >= projectileLifetime) {
        ...
        if (myPlayerId && pvpPlayers[myPlayerId]) {
          ...
          if (distanceSquared <= collisionRadiusSquared) {
            ...
            // Check for crit hit (using our crit chance - opponent's crit chance not synced)
            let isCritHit = Math.random() < critChance / 100;
            
            // Use opponent's damage stat (saved from projectile launch event)
            // Apply 2x damage (normal) or 3x damage (crit)
            const projectileDamage = isCritHit ? opponentDmg * 3 : opponentDmg * 2; // 2x normal, 3x crit
            ...
            // Apply damage to me (armor first, then HP)
            ...
            // Send MY stats update to opponent (they need to know my HP/Armor after taking damage)
            // This is MY player (myPlayerId), so send MY stats
      ...
      if (now - projectileSpawnTime >= projectileLifetime) {
        ...
        for (const playerId in pvpPlayers) {
          ...
          if (distanceSquared <= collisionRadiusSquared && playerId !== myPlayerId) {
            ...
            // Get my damage stat
            // Note: PvPPlayer doesn't have dmg property, use global dmg (same for all players)
            // Apply 2x damage (normal) or 3x damage (crit)
            const projectileDamage = isCritHit ? dmg * 3 : dmg * 2; // 2x normal, 3x crit
            ...
            // Apply damage to opponent (armor first, then HP)
            const absorbed = Math.min(projectileDamage, player.armor);
            player.armor -= absorbed;
            const remainingDamage = projectileDamage - absorbed;
            player.hp = Math.max(0, player.hp - remainingDamage);
            ...
            // NOTE: Do NOT send stats update here - opponent will calculate damage themselves
            // when they receive the projectile hit event. Sending opponent's stats here would
            // cause opponent to see their own HP decrease incorrectly.
    ...
    if (myPlayerId && opponentId && pvpPlayers[myPlayerId] && pvpPlayers[opponentId]) {
      ...
      if (distanceSquared < minDistanceSquared && distanceSquared > 0) {
        ...
        // The player with higher speed wins and deals damage to opponent
        {
          ...
          // Deal damage when players collide - the SLOWER player takes damage
          // Damage is dealt only if:
          // 1. Minimum speed is 5 (at least one player must have speed >= 5)
          // 2. Cooldown has passed (1 second since last collision damage)
          // 3. Damage is dealt only once per collision
          const now = Date.now();
          const COLLISION_DAMAGE_COOLDOWN = 1000; // 1 second
          const MIN_COLLISION_SPEED = 5; // Minimum speed for collision damage
          
          // Check if minimum speed requirement is met
          const maxSpeed = Math.max(speed1, speed2);
          const canDealDamage = maxSpeed >= MIN_COLLISION_SPEED;
          
          // Determine which player is slower (takes damage)
          let slowerPlayer: PvPPlayer | null = null;
          let fasterPlayer: PvPPlayer | null = null;
          let slowerSpeed = 0;
          ...
          // If speeds are equal, no damage is dealt
          
          // Deal damage to slower player if conditions are met
          if (slowerPlayer && fasterPlayer && canDealDamage) {
            // Check cooldown - use the slower player's lastCollisionDamageTime
            const timeSinceLastDamage = now - slowerPlayer.lastCollisionDamageTime;
            
            if (timeSinceLastDamage >= COLLISION_DAMAGE_COOLDOWN) {
              // Use base dmg from Solo stat (not 2x, just base dmg)
              const damage = dmg; // Base damage from Solo dmg stat
              const absorbed = Math.min(damage, slowerPlayer.armor);
              slowerPlayer.armor -= absorbed;
              const remainingDamage = damage - absorbed;
              slowerPlayer.hp = Math.max(0, slowerPlayer.hp - remainingDamage);
              
              // Update collision damage time
              slowerPlayer.lastCollisionDamageTime = now;
              
              // Show damage number
              safePushDamageNumber({
                x: slowerPlayer.x + (Math.random() - 0.5) * 40,
                y: slowerPlayer.y - 20,
                value: damage,
                life: 60,
                maxLife: 60,
              ...
              // No screen shake for collision damage
              
              const slowerPlayerId = slowerPlayer.id === myPlayerId ? 'Player 1' : 'Player 2';
              const fasterPlayerId = fasterPlayer.id === myPlayerId ? 'Player 1' : 'Player 2';
              console.log(`PvP Collision: ${fasterPlayerId} (speed ${Math.max(speed1, speed2).toFixed(2)}) collides with ${slowerPlayerId} (speed ${slowerSpeed.toFixed(2)}). ${slowerPlayerId} takes ${damage} damage. HP: ${slowerPlayer.hp}/${slowerPlayer.maxHP}, Armor: ${slowerPlayer.armor}/${slowerPlayer.maxArmor}`);
              
              // Check if slower player is dead
              if (slowerPlayer.hp <= 0) {
                slowerPlayer.isOut = true;
              }
            }
          }
