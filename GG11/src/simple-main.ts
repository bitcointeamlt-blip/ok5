...
const POSITION_SYNC_INTERVAL = 100; // Sync position every 100ms (10 times per second)
...
async function enterLobby(): Promise<void> {
  ...
  await supabaseService.clearOldMatches(walletState.address);

  // Enter lobby via matchmaking service
  const success = await matchmakingService.enterLobby(
    walletState.address,
    (match: Match, isPlayer1: boolean) => {
      // Match found! But don't start game yet - wait for both players to be ready
      currentMatch = match;
      isInLobby = false;
      isSearchingForMatch = false;
      isReady = false;
      ...
      console.log('Match found! Waiting for both players to be ready...', { matchId: match.id, isPlayer1 });
      
      // Subscribe to match updates to detect when opponent is ready
      if (walletState.address) {
        subscribeToMatchUpdates(match.id, walletState.address, isPlayer1);
      }
    }
  );
...
async function setPlayerReady(): Promise<void> {
  ...
  if (success) {
    isReady = true;
    console.log('Player ready set!', { matchId: currentMatch.id, isPlayer1 });
  }
}
...
function handleOpponentInput(input: any): void {
  ...
  if (input.type === 'position' && input.x !== undefined && input.y !== undefined) {
    // Directly update opponent position and velocity
    opponent.x = input.x;
    opponent.y = input.y;
...
function render() {
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    if (gameMode === 'PvP' && currentMatch && waitingForOpponentReady) {
      ...
      const walletState = walletService.getState();
      const isPlayer1 = currentMatch.p1 === walletState.address;
      const myReady = isPlayer1 ? (currentMatch.p1Ready || false) : (currentMatch.p2Ready || false);
      const opponentReady = isPlayer1 ? (currentMatch.p2Ready || false) : (currentMatch.p1Ready || false);
      ...
      if (!isReady) {
        const readyButtonX = canvas.width / 2 - 100;
        ...
        ctx.fillStyle = '#000000';
        ctx.font = 'bold 16px "Press Start 2P"';
        ctx.fillText('READY', canvas.width / 2, readyButtonY + readyButtonHeight / 2);
      } else {
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    if (gameMode === 'PvP' && opponentProjectileFlying) {
      ...
      if (now - opponentProjectileSpawnTime >= projectileLifetime) {
        ...
        // Opponent projectile collision with players (bouncing platform, NOT damage)
        for (const playerId in pvpPlayers) {
          const player = pvpPlayers[playerId];
          ...
          if (distance <= projectileRadius + player.radius) {
    ...
    if ((gameMode === 'PvP' || gameMode === 'Training') && projectileFlying) {
      ...
      if (now - projectileSpawnTime >= projectileLifetime) {
        ...
        // Projectile collision with players (bouncing platform, NOT damage)
        for (const playerId in pvpPlayers) {
          const player = pvpPlayers[playerId];
    ...
    if (gameMode === 'PvP' && currentMatch && myPlayerId && pvpPlayers[myPlayerId] && pvpSyncService.isSyncing()) {
      const now = Date.now();
      if (now - lastPositionSyncTime >= POSITION_SYNC_INTERVAL) {
        lastPositionSyncTime = now;
        const myPlayer = pvpPlayers[myPlayerId];
        
        // Send my position to opponent
        pvpSyncService.sendInput({
          type: 'position',
          timestamp: now,
          x: myPlayer.x,
