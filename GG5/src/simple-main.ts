...
async function enterLobby(): Promise<void> {
  ...
  if (!supabaseClient) {
    walletError = 'Supabase not configured. PvP Online requires Supabase setup.';
    console.error('Cannot enter lobby: Supabase client is null. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in .env file');
    return;
  }
  ...
  if (!success) {
    walletError = 'Failed to enter lobby. Check Supabase configuration.';
    isInLobby = false;
    isSearchingForMatch = false;
    console.error('Failed to enter lobby - matchmaking service returned false');
  }
}
...
canvas.addEventListener('mousemove', (e) => {
  ...
  // Check upgrade button hover - Solo mode only
        ...
        } else if (((gameMode === 'Solo' && (isHoveringUpgrade || isHoveringCrit || isHoveringAccuracy || hoveredLevel >= 0)) || isOverWallet || isHoveringGameMode || isHoveringPvPOnline) && !upgradeAnimation && !isDrawing) {
...
function gameLoop() {
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    // PvP/Training mode: Update projectile physics and collision
