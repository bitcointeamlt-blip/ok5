...
class MatchmakingService {
  ...
  async enterLobby(roninAddress: string, onMatchFound: MatchFoundCallback): Promise<boolean> {
    ...
    try {
      ...
      // Store interval ID for cleanup (validate key to prevent prototype pollution)
      const intervalKey = `matchPollInterval_${roninAddress}`;
      if (isSafeKey(intervalKey)) {
  ...
  async leaveLobby(roninAddress: string): Promise<boolean> {
    ...
    try {
      // Clear polling interval if exists (validate key to prevent prototype pollution)
      const intervalKey = `matchPollInterval_${roninAddress}`;
      if (isSafeKey(intervalKey)) {
        const pollInterval = (window as any)[intervalKey];
        if (pollInterval) {
          clearInterval(pollInterval);
