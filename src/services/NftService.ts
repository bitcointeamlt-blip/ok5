...
class NftService {
  ...
  async getTokenIds(walletAddress: string, balance: number): Promise<string[]> {
    try {
      ...
      try {
        ...
        console.log('tokensOfOwner not available, using tokenOfOwnerByIndex');
      }
      ...
      for (let i = 0; i < balance; i++) {
        try {
          ...
          console.log(`Token ${i}: ${tokenIdStr}`);
        } catch (error: any) {
          ...
          if (error.message && (error.message.includes('Too many requests') || error.code === 'SERVER_ERROR')) {
            console.log(`Rate limited at token index ${i}, waiting 2 seconds...`);
            ...
            try {
              ...
              const tokenIdStr = typeof tokenId === 'bigint' ? tokenId.toString() : String(tokenId);
              tokenIds.push(tokenIdStr);
              console.log(`Retry successful for token index ${i}: ${tokenIdStr}`);
    ...
    try {
      ...
      try {
        ...
        if (error.message && (error.message.includes('CORS') || error.message.includes('fetch'))) {
          console.log(`CORS error for token ${tokenId}, trying CORS proxy...`);
          ...
          try {
            ...
            metadata = await response.json();
            console.log(`Successfully fetched metadata via proxy for token ${tokenId}`);
      ...
      console.log(`Token ${tokenId} raw metadata image field:`, {
        ...
        try {
          ...
          if (metadataUrl.includes('s3.amazonaws.com')) {
            ...
            if (s3Match) {
              ...
              console.log(`Converted relative S3 image URL to: ${imageUrl}`);
            }
          }
        }
      }
      ...
      // Handle S3 URLs - they should work with crossOrigin='anonymous' in Image element
      if (imageUrl.includes('s3.amazonaws.com') || imageUrl.includes('s3.')) {
        console.log(`Token ${tokenId} has S3 image URL: ${imageUrl}`);
      }
      
      // Handle data URLs (base64 encoded images)
      if (imageUrl.startsWith('data:')) {
        console.log(`Token ${tokenId} has data URL image`);
      }
      
      console.log(`Token ${tokenId} final image URL:`, imageUrl || '(empty)');
  ...
  async loadPlayerNfts(walletAddress: string): Promise<NftItem[]> {
    try {
      ...
      console.log(`Found ${balance} NFTs`);
      ...
      for (let i = 0; i < tokenIds.length; i++) {
        ...
        try {
          ...
          console.log(`Loaded NFT ${i + 1}/${tokenIds.length}: ${nft.name}`);
        } catch (error: any) {
          ...
          if (error.message && error.message.includes('Too many requests')) {
            ...
            try {
              const nft = await this.getTokenMetadata(tokenId);
              nfts.push(nft);
              console.log(`Retry successful for token ${tokenId}`);
            } catch (retryError: any) {
      ...
      console.log(`Loaded ${nfts.length} NFTs (${nfts.filter(n => n.image).length} with images)`);
      return nfts;
    } catch (error: any) {
