...
const SLOT_SYMBOLS = ['DOT', 'DMG', 'CRIT', 'HP', 'ARMOR', '🎯', 'XP', 'X'] as const;
...
// Slot odds:
// - ARMOR reduced to 5%
// - The removed % from ARMOR was added to X
// => X = 39.285714...% ; ARMOR = 5% ; all other 6 symbols share the remaining equally (9.285714...% each)
const SLOT_P_ARMOR = 0.05;
const SLOT_P_X = 0.39285714285714285; // 39.2857%
const SLOT_P_OTHER = (1 - SLOT_P_ARMOR - SLOT_P_X) / 6; // DOT/DMG/CRIT/HP/🎯/XP
...
async function craftPewPewUfo(): Promise<void> {
  ...
  try {
    ...
    pewPewCraftStatus = 'Craft submitted!';
    setPewPewUfoCrafted(true); // TEMP until we move to on-chain ticket/SBT
  ...
  try {
    ...
    supabaseService.getProfile(pid).then((profile) => {
      try {
        ...
        if (typeof sd.currency === 'number' && Number.isFinite(sd.currency)) dotCurrency = Math.max(0, Math.round(sd.currency));
...
function render() {
  ...
  if (isPressingUpgrade) {
    ctx.fillStyle = UI_BTN_BG_PRESSED; // Darker when pressed
  } else if (isHoveringUpgrade) {
    ctx.fillStyle = UI_BTN_BG_HOVER; // Slightly darker when hovering
  } else {
    ctx.fillStyle = UI_BTN_BG; // Normal state
  }
  ...
  if (isPewPewOpen && !isLeaderboardOpen) {
    ...
    const lines = [
      'Token PvP mode is being built.',
      'Target rules:',
      '- Craft UFO Ticket: 200 RONKE (escrow)',
      '- Winner gets +100 RONKE',
      '- Loser loses the ticket',
      '',
    ...
    // Label - "UFO craft 200 $Ronke"
    ctx.fillStyle = UI_TEXT;
    ctx.font = 'bold 11px "Press Start 2P"';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    const craftLabel = pewPewCrafting ? 'CRAFTING...' : 'UFO craft 200 $Ronke';
...
if (!(window as any).__mousemoveListenerAdded) {
  ...
  } else if (((gameMode === 'Solo' && (isHoveringUpgrade || isHoveringCrit || isHoveringAccuracy || hoveredLevel >= 0)) || isOverContact || isOverPewPew || isOverProfile || isOverWallet || isHoveringContact || isHoveringPewPew || isHoveringPewPewCraft || isHoveringGameMode || isHoveringPvPOnline || isHoveringNewButton || isHoveringReady || isHoveringCancel) && !upgradeAnimation && !isDrawing) {
          canvas.style.cursor = 'pointer';
        } else if (isDrawing) {
          canvas.style.cursor = 'crosshair';
        } else if (mouseX > 240) {
