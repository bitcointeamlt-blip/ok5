...
function startPvPMatchOnServer(serverEndpoint?: string): void {
  // PvP Online requires UFO Ticket (RONKE-minted).
  ...
  try {
    ...
    if (!hasWallet) {
      walletError = 'Connect Ronin wallet (UFO Ticket required)';
      return;
    }
  } catch {}
...
const SOLO_SPIN_SHARDS_PER_SPIN = 10;
let soloSpins = 0;
let soloSpinMilestones = 0;
let soloSpinShards = 0;
...
function addSoloSpins(amount: number): { spinsAdded: number; shardsAdded: number } {
  const a = Math.max(0, Math.floor(amount));
  if (a <= 0) return { spinsAdded: 0, shardsAdded: 0 };
  ...
  const overflow = a - spinsAdded;
  const shardsAdded = overflow > 0 ? overflow * SOLO_SPIN_SHARDS_PER_SPIN : 0;
  if (shardsAdded > 0) soloSpinShards = Math.max(0, (soloSpinShards | 0) + shardsAdded);
  return { spinsAdded, shardsAdded };
}

function tryExchangeShardsForSpin(): boolean {
  if (slotIsSpinning) return false;
  if ((soloSpins | 0) >= SOLO_SPINS_CAP) return false;
  if ((soloSpinShards | 0) < SOLO_SPIN_SHARDS_PER_SPIN) return false;
  soloSpinShards = Math.max(0, (soloSpinShards | 0) - SOLO_SPIN_SHARDS_PER_SPIN);
  soloSpins = Math.min(SOLO_SPINS_CAP, (soloSpins | 0) + 1);
  forceSaveGame();
  return true;
}
...
function getSlotBellRect(): { x: number; y: number; w: number; h: number } {
  ...
  const bellW = 54; // widened to fit "SPIN" label
  const bellGap = 8;
...
function finishSlotSpinIfDone(now: number = Date.now()): void {
  ...
  if (same3 && a === 'X') {
    ...
    if (r.spinsAdded > 0 && r.shardsAdded > 0) payoutText = `+${r.spinsAdded} SPIN +${r.shardsAdded} SHARDS`;
    else if (r.spinsAdded > 0) payoutText = `+${r.spinsAdded} SPINS`;
    else payoutText = `+${r.shardsAdded} SHARDS`;
  } else if (!hasX && same3) {
...
async function craftPewPewUfo(): Promise<void> {
  ...
  try {
    if (hasUfoContract) {
      ...
      const ufoAbi = [
        'function mint((uint16 maxHP,uint16 maxArmor,uint16 dmg,uint8 critChance,uint8 accuracy) s) returns (uint256)',
        'function activeTokenIdOf(address owner) view returns (uint256)',
      ];
...
// Cache-bust so replacing er.png reloads without hard refresh.
const earthSpritePath = './er.png?v=1';
...
const EARTH_INTRO_DURATION_MS = 120_000;
let earthIntroActive = false;
let earthIntroStartAt = 0;
let earthIntroPlayedThisSession = false;

// Earth intro should be "one-time" only for fully registered players (Ronin wallet).
let earthIntroSeenKeyCached = '';
let earthIntroSeen = false;

function getWalletAddressForEarthIntro(): string | null {
  // "Registered" here means: connected via a real wallet address (0x...), not guest/auth.
  // This matches the request "prisijunge su pinigine" and avoids provider timing issues on startup.
  const isHex = (a: string) => /^0x[a-fA-F0-9]{40}$/.test(a);
  try {
    const st = walletService.getState();
    const addrLive = st?.address || null;
    if (addrLive && isHex(addrLive)) return addrLive;
  } catch {}
  try {
    const stored = localStorage.getItem('wallet_address') || localStorage.getItem('ronin_address');
    if (!stored) return null;
    if (stored.startsWith('auth_') || stored.startsWith('guest_')) return null;
    if (!isHex(stored)) return null;
    return stored;
  } catch {
    return null;
  }
...
function refreshEarthIntroSeenFromStorage(): void {
  try {
    const addr = getWalletAddressForEarthIntro();
    if (!addr) {
      earthIntroSeenKeyCached = '';
      earthIntroSeen = false;
      return;
    }
    const key = `dot_earth_intro_seen_v1:${addr}`;
    if (key !== earthIntroSeenKeyCached) {
      earthIntroSeenKeyCached = key;
      const raw = localStorage.getItem(key);
      earthIntroSeen = raw === '1' || raw === 'true';
    }

    // If we have a pending session mark (startup race), persist it now that we have addr.
    try {
      const pending = sessionStorage.getItem('dot_earth_intro_seen_pending') === '1';
      if (pending && !earthIntroSeen) {
        earthIntroSeen = true;
        localStorage.setItem(key, '1');
      }
      if (pending) sessionStorage.removeItem('dot_earth_intro_seen_pending');
    } catch {}
  } catch {
    earthIntroSeenKeyCached = '';
    earthIntroSeen = false;
...
function markEarthIntroSeen(): void {
  earthIntroSeen = true;
  try {
    // Only persist if this is a wallet-registered user.
    const addr = getWalletAddressForEarthIntro();
    if (!addr) {
      // Wallet state not restored yet; remember for this session and persist later.
      try { sessionStorage.setItem('dot_earth_intro_seen_pending', '1'); } catch {}
      return;
    }
    const key = `dot_earth_intro_seen_v1:${addr}`;
    earthIntroSeenKeyCached = key;
...
function updateEarthIntro(now: number): void {
  // Show only when the player starts Solo near Earth (distance ~ 0).
  if (gameMode !== 'Solo' || gameState !== 'Alive') return;
  // Wait for save/cloud/wallet to settle before deciding (prevents false triggers on load).
  if (now < (cinematicBootUntilMs || 0)) return;

  // If player is already far enough, never show Earth intro (requested threshold: 2k km).
  ...
  if (curDist > 2000) {
    ...
    // For wallet-registered players, persist "seen" so it never triggers later even if distance resets.
    try { if (getWalletAddressForEarthIntro()) markEarthIntroSeen(); } catch {}
    return;
  }

  // If user is Ronin-registered and already saw the intro once, never show it again.
  ...
  if (!earthIntroPlayedThisSession && soloDistanceKm <= 1) {
    // Show Earth immediately, but DON'T start movement until speed reaches 10 km/s (requested).
    earthIntroActive = true;
    earthIntroStartAt = 0; // 0 => movement not started yet
    earthIntroPlayedThisSession = true;
    // Persist "seen" immediately so refresh / distance reset won't replay it for Ronin users.
    markEarthIntroSeen();
  }
  if (!earthIntroActive) return;

  // Start moving once the player reaches 10 km/s.
  if (earthIntroStartAt === 0 && soloSpeedKmps >= 10) {
    earthIntroStartAt = now;
  }

  // Only end after movement has started and finished.
  if (earthIntroStartAt > 0) {
    const t = (now - earthIntroStartAt) / EARTH_INTRO_DURATION_MS;
    if (t >= 1) earthIntroActive = false;
  }
}
...
function drawEarthIntro(now: number): void {
  ...
  const tRaw = earthIntroStartAt > 0 ? ((now - earthIntroStartAt) / EARTH_INTRO_DURATION_MS) : 0;
  const t = clamp(tRaw, 0, 1);
  const tt = easeInOutCubic(t);
  ...
  // Big at start, then gently shrinks a bit as it fades (feels like you're moving away).
  const baseW = 980;
  const drawW = baseW * (1 - tt * 0.12);
  const drawH = drawW * aspect;

  // Start with >50% visible while still "from the left side".
  const startX = playLeft + drawW * 0.15;
  const endX = playLeft - drawW * 0.85;
  const x = startX + (endX - startX) * tt;
  const y = canvas.height * 0.55;

  const alpha = Math.max(0, 1 - tt) * 0.95;
  const rot = -0.02 + Math.sin(tt * Math.PI) * 0.01;
...
function loadGameState(saveData: SaveDataV2): void {
  ...
  // Global reset requested: ignore previous saved currency for everyone.
  dotCurrency = 0;
  ...
  soloSpinMilestones = (typeof (saveData as any)?.solo?.spinMilestones === 'number' && Number.isFinite((saveData as any).solo.spinMilestones))
    ...
    : 0;
  soloSpinShards = (typeof (saveData as any)?.solo?.spinShards === 'number' && Number.isFinite((saveData as any).solo.spinShards))
    ? Math.max(0, Math.floor((saveData as any).solo.spinShards))
    : 0;
  ...
  // Enforce cap on load; convert overflow spins into shards (so progress isn't lost).
  if (soloSpins > SOLO_SPINS_CAP) {
    const overflow = soloSpins - SOLO_SPINS_CAP;
    soloSpins = SOLO_SPINS_CAP;
    soloSpinShards += overflow * SOLO_SPIN_SHARDS_PER_SPIN;
  }
  ...
  try {
    ...
    supabaseService.getProfile(pid).then((profile) => {
      try {
        ...
        // Global reset requested: ignore cloud currency for everyone.
        dotCurrency = 0;
        ...
        if (typeof sd.spinShards === 'number' && Number.isFinite(sd.spinShards)) soloSpinShards = Math.max(0, Math.floor(sd.spinShards));
...
async function enterLobby(forcedEndpoint?: string): Promise<void> {
  ...
  try {
    ...
    room.onMessage("match_end", (msg: any) => {
      ...
      // Leave lobby after grace (server will also disconnect room).
      setTimeout(() => { void leaveLobby(); }, 5500);
    });
...
function render() {
  ...
  if (gameMode === 'Solo') {
    ...
    // Speed overlay (same style, live-updating)
    {
    ...
    ctx.save();
    // background plate for readability (taller frame: +23px top +23px bottom) (requested: +3px up/down)
    const platePadY = 23;
    ...
    const drawBtn = (bx: number, label: string, enabled: boolean) => {
      ...
      const cx = Math.round(bx + btnSize / 2);
      const cy = Math.round(btnY + btnSize / 2);
    ...
    ctx.strokeText(zText, btnMinusX - 10, mapY + 15);
    ctx.fillText(zText, btnMinusX - 10, mapY + 15);
    ctx.restore();
  ...
  ctx.strokeRect(2, 2, UI_PANEL_W - 4, canvas.height - 4);
    ...
    // Faucet button (+50 Rice) - usable only when Rice=0 and cooldown passed
    {
      ...
      if (!ready) ctx.fillStyle = 'rgba(120, 130, 150, 0.18)';
      else if (isPressingFaucet) ctx.fillStyle = UI_BTN_BG_PRESSED;
      else if (isHoveringFaucet) ctx.fillStyle = UI_BTN_BG_HOVER;
      ...
      ctx.font = 'bold 9px "Press Start 2P"';
      ctx.fillStyle = UI_TEXT;
      ctx.fillText('FAUCET +50', x + w / 2, y + 14);
      // Bottom label rules (requested):
      // - If cooldown is running, show time on the button
      // - Else if Rice > 0, show a message (not claimable)
      // - Else (Rice=0 and ready), no extra text (green frame indicates ready)
      let bottomText = '';
      if (remaining > 0) bottomText = formatCooldownShort(remaining);
      else if (hasRice) bottomText = 'RICE > 0';
      if (bottomText) {
        ctx.font = 'bold 7px "Press Start 2P"';
        ctx.fillStyle = UI_TEXT_MUTED;
        ctx.fillText(bottomText, x + w / 2, y + 30);
      }
      ctx.textAlign = 'left';
    }
  ...
  // Contact button (top-right)
  {
    ...
    const x = r.x;
    const y = r.y + (isPressingContact ? 1 : 0);
    const w = r.w;
    const h = r.h;
    // Match other panel buttons (shadow + highlight + pixel border)
    if (!isPressingContact) {
      ctx.fillStyle = UI_BTN_SHADOW;
      ctx.fillRect(x + 2, y + 2, w, h);
    }
    if (isPressingContact) ctx.fillStyle = UI_BTN_BG_PRESSED;
    else if (isHoveringContact || isContactOpen) ctx.fillStyle = UI_BTN_BG_HOVER;
    else ctx.fillStyle = UI_BTN_BG;
    ctx.fillRect(x, y, w, h);
    if (!isPressingContact) {
      ctx.fillStyle = UI_BTN_HILITE;
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('CONTACT', x + w / 2, y + h / 2 + 0.5);
    ctx.textAlign = 'left';
  }
  ...
  // PewPew button (above CONTACT)
  {
    ...
    const x = r.x;
    const y = r.y + (isPressingPewPew ? 1 : 0);
    const w = r.w;
    const h = r.h;
    // Match other big panel buttons (shadow + highlight + bevel)
    if (!isPressingPewPew) {
      ctx.fillStyle = UI_BTN_SHADOW;
      ctx.fillRect(x + 2, y + 2, w, h);
    }
    if (isPressingPewPew) ctx.fillStyle = UI_BTN_BG_PRESSED;
    else if (isHoveringPewPew || isPewPewOpen) ctx.fillStyle = UI_BTN_BG_HOVER;
    else ctx.fillStyle = UI_BTN_BG;
    ctx.fillRect(x, y, w, h);
    if (!isPressingPewPew) {
      ctx.fillStyle = UI_BTN_HILITE;
  ...
  if (!isPressingLeaderboard) {
    ctx.fillStyle = UI_BTN_SHADOW;
    ctx.fillRect(leaderboardButtonX + 2, leaderboardButtonY + 2, leaderboardButtonWidth, leaderboardButtonHeight);
  }
  if (isPressingLeaderboard) {
    ctx.fillStyle = UI_BTN_BG_PRESSED;
  } else if (isHoveringLeaderboard) {
    ctx.fillStyle = UI_BTN_BG_HOVER;
  } else {
    ctx.fillStyle = UI_BTN_BG;
  }
  ctx.fillRect(leaderboardButtonX, leaderboardButtonY, leaderboardButtonWidth, leaderboardButtonHeight);
  if (!isPressingLeaderboard) {
    ctx.fillStyle = UI_BTN_HILITE;
    ctx.fillRect(leaderboardButtonX, leaderboardButtonY, leaderboardButtonWidth, 2);
    ctx.fillRect(leaderboardButtonX, leaderboardButtonY, 2, leaderboardButtonHeight);
  }
  ctx.fillStyle = UI_BTN_BORDER;
  ctx.fillRect(leaderboardButtonX + leaderboardButtonWidth - 2, leaderboardButtonY, 2, leaderboardButtonHeight);
  ctx.fillRect(leaderboardButtonX, leaderboardButtonY + leaderboardButtonHeight - 2, leaderboardButtonWidth, 2);
  ctx.strokeStyle = UI_BTN_BORDER;
  ctx.lineWidth = 1;
  ctx.strokeRect(leaderboardButtonX, leaderboardButtonY, leaderboardButtonWidth, leaderboardButtonHeight);
  ctx.fillStyle = UI_TEXT;
  ctx.font = 'bold 10px "Press Start 2P"';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('LEADERBOARD', leaderboardButtonX + leaderboardButtonWidth / 2, leaderboardButtonY + leaderboardButtonHeight / 2);
  ...
  // Button highlight (subtle) - only if not pressed
  if (!isPressingProfile) {
    ctx.fillStyle = UI_BTN_HILITE;
  ...
  try {
    ...
    if (walletConnecting) {
      ...
      ctx.font = 'bold 6px "Press Start 2P"';
      ctx.fillText(display, walletButtonX + walletButtonWidth/2, walletButtonY + walletButtonHeight/2 - 5);
  ...
  // Button border
  ctx.strokeStyle = (gameMode === 'Training') ? UI_ACCENT_YELLOW : UI_BTN_BORDER;
  ctx.lineWidth = 1;
  ...
  // Button border
  ctx.strokeStyle = (gameMode === 'PvP') ? UI_ACCENT_GREEN : UI_BTN_BORDER;
  ctx.lineWidth = 1;
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    if (gameMode === 'PvP' && !isInLobby && !currentMatch && (!myPlayerId || !pvpPlayers[myPlayerId])) {
      ...
      ctx.fillStyle = UI_TEXT_MUTED;
      ctx.font = 'bold 12px "Press Start 2P"';
      ctx.fillText('Click PvP Online button to retry', cardX + cardW / 2, cardY + cardH - 42);
...
if (!(window as any).__mousemoveListenerAdded) {
  ...
  try {
    ...
    if (isChatOpen) {
      ...
      if (hit) {
        ...
        const shouldPointer = isHoveringChat || isHoveringSlotBell || isHoveringChatSend || isHoveringChatClose || overName || isHoveringNicknameEdit || isHoveringSpeedFxMinus || isHoveringSpeedFxPlus || isHoveringFaucet;
  ...
  } else if (((gameMode === 'Solo' && (hoveredLevel >= 0)) || isOverFaucet || isOverContact || isOverPewPew || isOverProfile || isOverWallet || isHoveringFaucet || isHoveringContact || isHoveringPewPew || isHoveringPewPewCraft || isHoveringGameMode || isHoveringPvPOnline || isHoveringNewButton || isHoveringReady || isHoveringCancel) && !upgradeAnimation && !isDrawing) {
          canvas.style.cursor = 'pointer';
        } else if (isDrawing) {
          canvas.style.cursor = 'crosshair';
        } else if (mouseX > 240) {
...
if (!(window as any).__clickListenerAdded) {
  ...
  if (slotModalOpen && e.button === 0) {
    ...
    if (r && typeof r.x === 'number') {
      ...
      const inExch =
        mouseX >= r.exchX && mouseX <= r.exchX + r.exchW &&
        mouseY >= r.exchY && mouseY <= r.exchY + r.exchH;
      if (inExch) {
        const ok = tryExchangeShardsForSpin();
        try { ok ? audioManager.playUpgradeSuccess() : audioManager.playUpgradeFail(); } catch {}
        return;
      }
...
function gameLoop() {
  ...
  if (gameMode === 'Solo' && gameState === 'Alive') {
    ...
    // Cap spins at SOLO_SPINS_CAP; overflow becomes shards (10 shards = 1 spin via exchange).
    ...
    if (milestones > soloSpinMilestones) {
      ...
      try {
        safePushDamageNumber({
          ...
          y: 120,
          value:
            (r.spinsAdded > 0 && r.shardsAdded > 0) ? `SPIN +${r.spinsAdded} | SHARDS +${r.shardsAdded}` :
            (r.spinsAdded > 0) ? `SPIN +${r.spinsAdded}` :
            `SHARDS +${r.shardsAdded}`,
          life: 90,
          maxLife: 90,
          vx: 0,
          vy: -1.2,
  ...
  // Slot "SPIN" button + next spin info (left of LIVE CHAT)
  if (gameMode === 'Solo') {
    const r = getSlotBellRect();
    const bellX = r.x;
    const bellY = r.y;
    ...
    // Button background (match LIVE CHAT style: translucent)
    const now = Date.now();
    const spinHasNew = slotBellNewSpins > 0 || now < (slotSpinGlowUntil | 0);
    const spinPulseK = spinHasNew ? (0.55 + 0.45 * Math.sin(now / 180)) : 0;
    ctx.save();
    ctx.globalAlpha = 0.5;
    ctx.fillStyle = spinHasNew
      ? `rgba(80, 200, 120, ${0.45 + spinPulseK * 0.35})`
      : (isHoveringSlotBell ? 'rgba(18, 22, 36, 0.70)' : 'rgba(8, 10, 18, 0.55)');
    ctx.fillRect(bellX, bellY, bellW, btnH);
    ctx.restore();
    ctx.save();
    ctx.strokeStyle = spinHasNew ? 'rgba(160, 255, 200, 0.35)' : 'rgba(210, 235, 255, 0.18)';
    ctx.lineWidth = 1;
    ctx.strokeRect(bellX, bellY, bellW, btnH);
    ctx.restore();
    ...
    // Label: SPIN
    {
      ...
      ctx.lineWidth = 4;
      ctx.strokeStyle = 'rgba(0, 0, 0, 0.85)';
      ctx.fillStyle = spinHasNew ? '#b6ffd0' : '#ff9a00';
      ctx.font = 'bold 9px "Press Start 2P"';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.strokeText('SPIN', bellX + bellW / 2, bellY + btnH / 2 + 0.5);
      ctx.fillText('SPIN', bellX + bellW / 2, bellY + btnH / 2 + 0.5);
      ctx.restore();
    }
    ...
    // NEXT info plate next to the SPIN button
    {
      const dist = Math.max(0, Number.isFinite(soloDistanceKm) ? soloDistanceKm : 0);
      ...
      const rewardLabel = (spinsNow >= SOLO_SPINS_CAP) ? `+${SOLO_SPIN_SHARDS_PER_SPIN} SHARDS` : '+1 SPIN';
      const nextText = `NEXT: ${rewardLabel} IN ~${Math.ceil(remKm).toFixed(0)} KM`;

      ctx.save();
      ctx.font = 'bold 9px "Press Start 2P"';
      const padX = 10;
      const tw = ctx.measureText(nextText).width;
      const infoW = Math.min(320, Math.max(170, tw + padX * 2));
      const infoGap = 6;
      // Align with the "front" (left edge) of the SPIN button for a cleaner look.
      const infoX = Math.max(10, Math.min(canvas.width - 10 - infoW, Math.floor(bellX)));
      const infoY = bellY + btnH + infoGap;
      ctx.globalAlpha = 0.5;
      ctx.fillStyle = 'rgba(8, 10, 18, 0.55)';
      ctx.fillRect(infoX, infoY, infoW, btnH);
      ctx.globalAlpha = 1;
      ctx.strokeStyle = 'rgba(210, 235, 255, 0.18)';
      ctx.lineWidth = 1;
      ctx.strokeRect(infoX, infoY, infoW, btnH);
      ctx.lineJoin = 'round';
      ctx.lineWidth = 4;
      ctx.strokeStyle = 'rgba(0, 0, 0, 0.85)';
      ...
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.strokeText(nextText, infoX + padX, infoY + btnH / 2 + 0.5);
      ctx.fillText(nextText, infoX + padX, infoY + btnH / 2 + 0.5);
      ctx.restore();
    }
  }
  ...
  if (slotModalOpen) {
    ...
    const drawReel = (i: number) => {
      ...
      const symText = (symRaw === 'RICE') ? 'Rice' : symRaw;
      // Visual request: make X red.
      ctx.fillStyle = (symRaw === 'X') ? '#ff4d4d' : '#ffffff';
    ...
    ctx.font = 'bold 10px "Press Start 2P"';
    const spinsNow = Math.max(0, soloSpins | 0);
    const shardsNow = Math.max(0, soloSpinShards | 0);
    const spinsGenNow = Math.max(0, soloSpinsGeneratedTotal | 0);
    ctx.fillText(`SPINS: ${spinsNow}/${SOLO_SPINS_CAP}   SHARDS: ${shardsNow}`, modalX + modalW / 2, modalY + 75);
    ctx.font = 'bold 10px "Press Start 2P"';
    ctx.fillStyle = 'rgba(230, 240, 255, 0.70)';
    ctx.fillText(`TOTAL GENERATED: ${spinsGenNow}`, modalX + modalW / 2, modalY + 100);
    ...
    ctx.fillStyle = slotLastPayoutText ? '#39ff6a' : '#ffffff';
    ctx.font = slotLastPayoutText ? 'bold 14px "Press Start 2P"' : 'bold 9px "Press Start 2P"';
    ctx.fillText(msg, modalX + modalW / 2, modalY + 285);
    ...
    const exchW = 300;
    const exchH = 28;
    const exchX = modalX + Math.floor(modalW / 2 - exchW / 2);
    const exchY = modalY + 110;
    ...
    ctx.font = 'bold 11px "Press Start 2P"';
    ctx.fillText(boostEnabled ? 'SPIN (-10)' : 'SPIN', spinX + btnW / 2, spinY + btnH / 2 + 1);
    ...
    ctx.fillStyle = canToggle ? boostBg : 'rgba(120, 120, 120, 0.25)';
    ctx.fillRect(boostX, boostY, btnW, btnH);
    ctx.strokeStyle = 'rgba(210, 235, 255, 0.22)';
    ctx.lineWidth = 2;
    ctx.strokeRect(boostX, boostY, btnW, btnH);
    ctx.fillStyle = canToggle ? (boostEnabled ? '#9fe6ff' : '#9fe6ff') : 'rgba(230, 240, 255, 0.55)';
    ctx.font = 'bold 9px "Press Start 2P"';
    ctx.fillText('2X', boostX + btnW / 2, boostY + btnH / 2 + 1);

    // Exchange button (10 shards -> 1 spin)
    const canExchange = (!slotIsSpinning) && shardsNow >= SOLO_SPIN_SHARDS_PER_SPIN && spinsNow < SOLO_SPINS_CAP;
    ctx.fillStyle = canExchange ? 'rgba(20, 28, 48, 0.70)' : 'rgba(120, 120, 120, 0.18)';
    ctx.fillRect(exchX, exchY, exchW, exchH);
    ctx.strokeStyle = 'rgba(210, 235, 255, 0.18)';
    ctx.lineWidth = 1;
    ctx.strokeRect(exchX, exchY, exchW, exchH);
    ctx.fillStyle = canExchange ? '#ffffff' : 'rgba(230, 240, 255, 0.55)';
    ctx.font = 'bold 8px "Press Start 2P"';
    ctx.fillText(`EXCHANGE ${SOLO_SPIN_SHARDS_PER_SPIN} -> +1 SPIN`, exchX + exchW / 2, exchY + exchH / 2 + 1);
