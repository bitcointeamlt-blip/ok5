      ...
      if (gameMode === 'Solo' && soloEncounterPhase !== 'warning' && soloEncounterPhase !== 'fled') {
        ...
        if (ufoSprite && ufoSprite.complete) {
          try {
            ...
            const spriteToDraw = ufoSpriteProcessed || ufoSprite;
...
function gameLoop() {
  ...
  if (gameMode === 'Solo' && gameState === 'Alive') {
    ...
    // Space encounters: queue pending fights by distance milestone, then start one when possible.
    if (soloEncounterBaselineInited && !soloEncounterInProgress) {
      ...
      if (soloEncounterPending > 0 && (now - (soloEncounterJustEndedAt || 0)) > 1200) {
        ...
        startSoloEncounter();
        // Reset perf baseline so distance doesn't “jump” on return.
        soloDistanceLastPerfMs = frameStartTime;
      }
    }
  } else {
