...
function render() {
  ...
  try {
    ...
    const balanceFrameHeight = 58; // DOT + NFT status
...
if (!(window as any).__mousemoveListenerAdded) {
  ...
  } else if (((gameMode === 'Solo' && (isHoveringUpgrade || isHoveringCrit || isHoveringAccuracy || hoveredLevel >= 0)) || isOverContact || isOverProfile || isOverWallet || isHoveringContact || isHoveringGameMode || isHoveringPvPOnline || isHoveringNewButton || isHoveringReady || isHoveringCancel) && !upgradeAnimation && !isDrawing) {
          canvas.style.cursor = 'pointer';
        } else if (isDrawing) {
          canvas.style.cursor = 'crosshair';
        } else if (mouseX > 240) {
    ...
    if (mouseX >= closeButtonX && mouseX <= closeButtonX + closeButtonSize &&
      ...
      if (nicknameInput) {
        ...
        if (nicknameInput.value.trim()) {
          ...
          // Also save to Supabase snapshot (works even if DB has no `nickname` column)
          const pid = getRoninAddressForPersistence();
          if (pid) {
            const p = profileManager.getProfile();
