...
function render() {
  ...
  if (gameMode === 'PvP' || gameMode === 'Training') {
    ...
    for (const playerId in pvpPlayers) {
      ...
      // Draw fading shadow tail (behind the player)
      if (PVP_UFO_SPEED_TRAIL_ENABLED) {
        ...
        ctx.save();
        const SHADOW_TAIL_ALPHA = 0.14; // lower = lighter/softer shadow
      ...
      if (player.facingLeft) {
        if (player.vx > VX_FACE_ON) player.facingLeft = false;
        } else {
        if (player.vx < -VX_FACE_ON) player.facingLeft = true;
      }
      ...
      let targetAngle = Math.atan2(player.vy, denom);
      if (speed < 0.12) targetAngle = 0;

      // Smooth it (visual only)
      player.renderAngle = lerpAngle(player.renderAngle || 0, targetAngle, 0.18);
