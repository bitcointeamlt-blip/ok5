...
if (!(window as any).__mouseupListenerAdded) {
  ...
  if (isChatOpen) {
    ...
    // Click username -> open profile
    for (const r of chatHitRegions) {
      if (mx >= r.x && mx <= r.x + r.w && my >= r.y && my <= r.y + r.h) {
        chatSelectedUserSessionId = r.sessionId || null;
        chatSelectedUserAddress = r.address || null;
        opponentAddressForResult = r.address || '';
        isOpponentProfileOpen = true;
        opponentProfileData = null;
        if (supabaseService.isConfigured() && r.address) {
          supabaseService.getProfile(r.address).then((p) => { opponentProfileData = p; }).catch(() => {});
        }
        return;
      }
    }
...
function gameLoop() {
  ...
  if (isChatOpen) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const text = m.kind === 'system' ? `${name} ${m.text}` : m.text;
      const color = m.kind === 'system' ? 'rgba(230, 240, 255, 0.68)' : '#ffffff';
      const nameColor = '#b04cff';
      const ts = new Date(m.ts || Date.now());
      const tsText = ts.toLocaleDateString('lt-LT');
      ...
      // Message text
      ctx.fillStyle = color;
      ctx.fillText(m.kind === 'system' ? '' : text, listX + Math.min(240, nameW + 14), y);

      // Right timestamp
      ctx.fillStyle = 'rgba(230, 240, 255, 0.45)';
      ctx.textAlign = 'right';
      ctx.fillText(tsText, listX + listW, y);
