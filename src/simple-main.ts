...
function render() {
  ...
  if (isLeaderboardOpen) {
    ...
    if (leaderboardLoading) {
      ...
      ctx.fillText('W', contentX + 520, headerY);
      ctx.fillText('L', contentX + 590, headerY);
      ctx.fillText('MAX HP', contentX + 650, headerY);
      ...
      for (let i = start; i < end; i++) {
        ...
        ctx.fillText(String(r.wins ?? 0), contentX + 520, y);
        ctx.fillText(String(r.losses ?? 0), contentX + 590, y);
        ctx.fillText(String(r.maxHP ?? 10), contentX + 680, y);
        y += rowH;
      }
    }
