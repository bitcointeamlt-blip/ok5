...
class Game {
  ...
  constructor() {
    ...
    this.droneSystem = new DroneSystem(this.gameState, this.combatSystem);
    this.particleSystem = new ParticleSystem();
    this.uiRenderer = new UIRenderer(this.gameState, this.upgradeSystem);
    this.uiRenderer.setDroneSystem(this.droneSystem);
    ...
    this.renderer.setDroneSystem(this.droneSystem);
  ...
  private update(deltaTime: number): void {
    ...
    // Update drones (movement and attacks)
  ...
  private handleDOTDeath(): void {
    if (this.gameState.flags.state === 'Dying' && this.gameState.timers.death <= 0) {
      // Add collector drone bonus before respawn
      const collectorBonus = this.droneSystem.getCollectorBonus();
      if (collectorBonus > 0) {
        this.gameState.player.dotCurrency += collectorBonus;
        this.uiRenderer.showFeedback(`+${collectorBonus} Collector Bonus!`, '#44ff44', 1.5);
      }
  ...
  // Get drone system for external access
  public getDroneSystem(): DroneSystem {
    return this.droneSystem;
  }
}
