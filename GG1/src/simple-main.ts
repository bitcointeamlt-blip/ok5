...
// PvP Projectile system (Angry Birds style)
...
async function enterLobby(): Promise<void> {
  ...
  // Enter lobby via matchmaking service
  const success = await matchmakingService.enterLobby(
    walletState.address,
    (match: Match, isPlayer1: boolean) => {
      // Match found!
      currentMatch = match;
      isInLobby = false;
      isSearchingForMatch = false;
...
async function leaveLobby(): Promise<void> {
  ...
  await matchmakingService.leaveLobby(walletState.address);
  isInLobby = false;
  isSearchingForMatch = false;
  currentMatch = null;
}
...
function initializePvPWithMatch(match: Match, isPlayer1: boolean): void {
  ...
  // Initialize opponent (will be updated via network sync)
  const opponent: PvPPlayer = {
    id: opponentId,
    x: pvpBounds.left + (pvpBounds.right - pvpBounds.left) * 0.75, // Right quarter
    y: pvpBounds.bottom / 2,
  ...
  console.log('PvP initialized with match', { matchId: match.id, isPlayer1, seed: match.seed });
}

// Handle opponent input from network
function handleOpponentInput(input: any): void {
  if (!opponentId || !pvpPlayers[opponentId]) return;
  ...
  // TODO: Handle arrow and projectile inputs
}
...
function render() {
  ...
  if (gameMode === 'Solo') {
    if (arrowReady && !arrowFired && gameState === 'Alive') {
      ...
      ctx.font = 'bold 8px "Press Start 2P"';
      ctx.textAlign = 'left';
      ctx.fillText('ARROW READY - Click to fire!', 20, 340);
    } else if (arrowFired && gameState === 'Alive') {
      // Show arrow already fired
      ctx.fillStyle = '#808080'; // Gray when fired
      ctx.font = 'bold 8px "Press Start 2P"';
      ctx.textAlign = 'left';
      ctx.fillText('ARROW FIRED', 20, 340);
    }
  }

  // Wallet connection button (above Game Mode button) - ALWAYS render, regardless of game mode
  const walletButtonX = 20;
  const walletButtonY = 310; // Above Game Mode button
  ...
  const gameModeButtonX = 20;
  const gameModeButtonY = 360; // Below wallet button
  const gameModeButtonWidth = 200;
  const gameModeButtonHeight = 40;
  
  // Button shadow (dark gray) - only if not pressed
  if (!isPressingGameMode) {
    ctx.fillStyle = '#404040';
    ctx.fillRect(gameModeButtonX + 2, gameModeButtonY + 2, gameModeButtonWidth, gameModeButtonHeight);
  }
  ...
  if (isPressingGameMode) {
    ...
    ctx.fillStyle = gameMode === 'PvP' ? '#ffff00' : '#c0c0c0'; // Yellow for PvP, gray for Solo
  }
  ctx.fillRect(gameModeButtonX, gameModeButtonY, gameModeButtonWidth, gameModeButtonHeight);
  
  // Button highlight (white) - only if not pressed
  if (!isPressingGameMode) {
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(gameModeButtonX, gameModeButtonY, gameModeButtonWidth, 2);
    ctx.fillRect(gameModeButtonX, gameModeButtonY, 2, gameModeButtonHeight);
  }
  
  // Button shadow (dark gray)
  ctx.fillStyle = '#808080';
  ctx.fillRect(gameModeButtonX + gameModeButtonWidth - 2, gameModeButtonY, 2, gameModeButtonHeight);
  ctx.fillRect(gameModeButtonX, gameModeButtonY + gameModeButtonHeight - 2, gameModeButtonWidth, 2);
  
  // Button border (black)
  ctx.strokeStyle = '#000000';
  ctx.lineWidth = 1;
  ctx.strokeRect(gameModeButtonX, gameModeButtonY, gameModeButtonWidth, gameModeButtonHeight);
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(`MODE: ${gameMode}`, gameModeButtonX + gameModeButtonWidth/2, gameModeButtonY + gameModeButtonHeight/2);
  ...
  // PvP mode: Lobby screen or Arena
  if (gameMode === 'PvP') {
    // Show lobby screen if searching for match
    if (isInLobby && !currentMatch) {
    ...
    for (const playerId in pvpPlayers) {
      ...
      // Draw opponent HP and Armor stats (near opponent)
      if (playerId === opponentId) {
        // Opponent stats - draw near opponent
        const statsX = player.x - 100; // Left of opponent
        const statsY = player.y - player.radius - 60; // Above opponent
        const statsWidth = 200;
        const statsHeight = 50;
      ...
      // Draw player ID label (if opponent)
      if (playerId === opponentId) {
        ctx.fillStyle = '#000000';
        ctx.font = 'bold 8px "Press Start 2P"';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'bottom';
        ctx.fillText('OPPONENT', player.x, player.y - player.radius - 5);
    ...
    // PvP mode: Draw flying projectile
    if (projectileFlying) {
...
canvas.addEventListener('mousemove', (e) => {
  ...
  isHoveringCrit = isOverCrit && !upgradeAnimation && !isDrawing;
        ...
        // Check katana button hover
        // Arrow ready tracking removed - no button hover needed
        ...
        const isOverWallet = mouseX >= 20 && mouseX <= 220 && mouseY >= 310 && mouseY <= 350;
        isHoveringWallet = isOverWallet && !upgradeAnimation && !isDrawing;

        // Check game mode button hover
        const isOverGameMode = mouseX >= 20 && mouseX <= 220 && mouseY >= 360 && mouseY <= 400;
        isHoveringGameMode = isOverGameMode && !upgradeAnimation && !isDrawing;

        // Change cursor - show arrow icon when arrow is ready
        if (arrowReady && !arrowFired && mouseX > 240 && !isDrawing && gameState === 'Alive') {
          // Custom arrow cursor
          canvas.style.cursor = 'crosshair';
        } else if ((isOverUpgrade || isOverCrit || isOverAccuracy || hoveredLevel >= 0 || isOverWallet || isOverGameMode) && !upgradeAnimation && !isDrawing) {
...
canvas.addEventListener('mousedown', (e) => {
  ...
  // Check if clicking level button
  for (const btn of levelButtons) {
    if (mouseX >= btn.x && mouseX <= btn.x + 32 && mouseY >= btn.y && mouseY <= btn.y + 32) {
      const levelDiff = currentLevel - btn.level;
      const canAccess = btn.level <= maxUnlockedLevel && levelDiff <= 2 && levelDiff >= 0;
  ...
  if (isOverGameMode) {
    ...
    return; // Don't start drawing if clicking game mode button
  }
...
canvas.addEventListener('mouseup', (e) => {
            ...
            .then(async (result) => {
              if (result) {
                ...
                if (!authResult.success) {
                  walletError = authResult.error || 'Auth failed';
  ...
  if (pressedLevel >= 0 && pressedLevel !== currentLevel) {
  ...
  if (isPressingGameMode) {
    ...
    if (isOverGameMode) {
      // Toggle game mode
      const newMode: GameMode = gameMode === 'Solo' ? 'PvP' : 'Solo';
      
      if (newMode === 'PvP' && gameMode === 'Solo') {
        // Switching to PvP - initialize PvP
        gameMode = 'PvP';
        initializePvP();
        console.log('Switched to PvP mode');
      } else if (newMode === 'Solo' && gameMode === 'PvP') {
        // Switching to Solo - cleanup PvP
        gameMode = 'Solo';
        cleanupPvP();
...
window.addEventListener('keydown', (e) => {
  ...
  // PvP mode: Arrow activation
  if (e.key === '1' && gameMode === 'PvP' && myPlayerId && pvpPlayers[myPlayerId]) {
    const currentTime = Date.now();
    const timeSinceLastShot = currentTime - pvpArrowLastShotTime;
  ...
  // PvP mode: Start charging projectile (key "2")
  if (e.key === '2' && gameMode === 'PvP' && myPlayerId && pvpPlayers[myPlayerId] && !projectileFlying && !projectileCharging) {
    const myPlayer = pvpPlayers[myPlayerId];
...
window.addEventListener('keyup', (e) => {
  // PvP mode: Launch projectile if charging (key "2" released)
  if (e.key === '2' && gameMode === 'PvP' && projectileCharging && !projectileFlying && myPlayerId && pvpPlayers[myPlayerId]) {
    const chargeTime = Date.now() - projectileChargeStartTime;
...
canvas.addEventListener('click', (e) => {
  ...
  // PvP mode: Handle arrow throw - click to launch arrow (only if ready and cooldown passed)
  if (gameMode === 'PvP' && pvpArrowReady && mouseX > 240 && myPlayerId && pvpPlayers[myPlayerId] && !pvpKatanaFlying && e.button === 0) {
    const currentTime = Date.now();
    const timeSinceLastShot = currentTime - pvpArrowLastShotTime;
  ...
  // PvP mode: Click handler (same as Solo DOT click system with accuracy/miss)
  if (gameMode === 'PvP' && myPlayerId && pvpPlayers[myPlayerId]) {
    const rect = canvas.getBoundingClientRect();
    const mouseX = e.clientX - rect.left;
    const mouseY = e.clientY - rect.top;
    ...
    if (mouseX > 240 && e.button === 0) {
      ...
      if (distance <= myPlayer.radius) {
        ...
        if (isHit) {
          ...
          if (currentMatch && pvpSyncService.isSyncing()) {
            pvpSyncService.sendInput({
              type: 'click',
              timestamp: Date.now(),
              x: mouseX,
              y: mouseY,
...
function gameLoop() {
    ...
    // Drawn lines collision (pencil tool - check all segments)
    {
      for (let i = drawnLines.length - 1; i >= 0; i--) {
        ...
        for (let j = 0; j < line.points.length - 1; j++) {
          ...
          if (len < 0.1) continue; // Skip degenerate segments
          
          // Distance from DOT center to line segment
          const t = Math.max(0, Math.min(1, ((dotX - p1.x) * dx + (dotY - p1.y) * dy) / (len * len)));
          ...
          // Collision if DOT is close to line and moving towards it
          const lineThickness = 3;
          if (distance <= dotRadius + lineThickness / 2 && dotVy > 0) {
        ...
        if (collisionFound) {
          ...
          // Speed >= 7 bonus damage (same as platform)
          const speedNow = Math.sqrt(dotVx * dotVx + dotVy * dotVy);
          if (Math.round(speedNow) >= 7) {
    ...
    // Moving platform collision (straight platform)
    {
      ...
      if (dotX >= movingPlatformX - halfW - dotRadius && dotX <= movingPlatformX + halfW + dotRadius) {
        if (dotY + dotRadius >= surfaceY && dotVy > 0) {
          ...
          // If current speed is >=7, award a force-crit for next hit and bonus damage
          const speedNow = Math.sqrt(dotVx * dotVx + dotVy * dotVy);
          if (Math.round(speedNow) >= 7) {
            nextHitForceCrit = true;
            nextHitForceCritExpiresAt = Date.now() + 1500;
  ...
  // PvP mode: Physics update for all players (copied from Solo DOT physics)
  if (gameMode === 'PvP') {
    for (const playerId in pvpPlayers) {
      const player = pvpPlayers[playerId];
    ...
    // PvP mode: Update arrow physics and collision
    if (gameMode === 'PvP' && pvpKatanaFlying && opponentId && pvpPlayers[opponentId]) {
      ...
      const hitRadius = opponent.radius + (arrowLength - arrowFletchingLength); // Arrow body length + opponent radius
    ...
    // PvP mode: Update projectile physics and collision
    if (gameMode === 'PvP' && projectileFlying) {
      ...
      if (opponentId && pvpPlayers[opponentId]) {
        const opponent = pvpPlayers[opponentId];
        const dx = projectileX - opponent.x;
        const dy = projectileY - opponent.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        
        if (distance <= projectileRadius + opponent.radius) {
          // Hit! Deal 3x damage and push opponent down
          const projectileDamage = dmg * 3; // 3x damage
          
          // Apply damage (armor first, then HP)
          const absorbed = Math.min(projectileDamage, opponent.armor);
          opponent.armor -= absorbed;
          const remainingDamage = projectileDamage - absorbed;
          opponent.hp = Math.max(0, opponent.hp - remainingDamage);
          
          // Push opponent down with speed 10
          opponent.vy = 10; // Strong push down
          ...
          console.log(`Projectile hit opponent! Damage: ${projectileDamage}, HP: ${opponent.hp}/${opponent.maxHP}`);
    ...
    // PvP mode: Update each player's physics (including drawn lines and platform collision)
    if (gameMode === 'PvP') {
      for (const playerId in pvpPlayers) {
      const player = pvpPlayers[playerId];
      ...
      // Drawn lines collision (pencil tool - same as Solo)
      {
        for (let i = drawnLines.length - 1; i >= 0; i--) {
          ...
          for (let j = 0; j < line.points.length - 1; j++) {
            ...
            // Distance from player center to line segment
            const t = Math.max(0, Math.min(1, ((player.x - p1.x) * dx + (player.y - p1.y) * dy) / (len * len)));
            const closestX = p1.x + t * dx;
            const closestY = p1.y + t * dy;
            ...
            // Collision if player is close to line and moving towards it
            const lineThickness = 3;
            if (distance <= player.radius + lineThickness / 2 && player.vy > 0) {
              // Calculate normal to line (perpendicular)
              const nx = -dy / len;
              const ny = dx / len;
      ...
      // Update speed trail (supersonic animation - same as Solo)
      const speed = Math.sqrt(player.vx * player.vx + player.vy * player.vy);
      if (Math.round(speed) >= 7) {
    ...
    // PvP mode: Check win/loss conditions
    ...
    // Bot AI: Control red player (opponent) only
    ...
    if (myPlayerId && opponentId && pvpPlayers[myPlayerId] && pvpPlayers[opponentId]) {
      ...
      if (distance < minDistance && distance > 0) {
        ...
        {
          // Determine which player dominates (faster speed)
  ...
  if (katanaFlying && gameState === 'Alive') {
    ...
    const hitRadius = dotRadius + (arrowLength - arrowFletchingLength); // Arrow body length + DOT radius
    if (distance <= hitRadius) {
  ...
  // Update supersonic shadow tail (comet effect)
  {
    const speed = Math.sqrt(dotVx * dotVx + dotVy * dotVy);
    if (Math.round(speed) >= 7) {
      // Add a new segment at current position
