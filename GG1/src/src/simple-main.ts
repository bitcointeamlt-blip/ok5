...
const BALANCE_CHECK_INTERVAL = 10000; // Check balance every 10 seconds
let dotHP = 10;
let dotMaxHP = 10;
let dotArmor = 5;
let dotMaxArmor = 5;
...
function render() {
  ...
  // Game Mode toggle button (moved down to avoid overlapping with other info)
  const gameModeButtonX = 20;
  let gameModeButtonY = 500; // Default position
  try {
    const walletStateForButton = walletService.getState();
    if (walletStateForButton.isConnected && walletStateForButton.address) {
      gameModeButtonY = walletButtonY + walletButtonHeight + 60; // Below balance frame
    }
  } catch (error) {
...
canvas.addEventListener('mousemove', (e) => {
  ...
  // Check upgrade button hover - Solo mode only
        ...
        const isOverGameMode = mouseX >= 20 && mouseX <= 220 && mouseY >= 500 && mouseY <= 540;
        ...
        } else if (((gameMode === 'Solo' && (isHoveringUpgrade || isHoveringCrit || isHoveringAccuracy || hoveredLevel >= 0)) || isOverWallet || isOverGameMode) && !upgradeAnimation && !isDrawing) {
