#!/usr/bin/env python3
"""
Create GG10.zip with Unix path separators for Netlify deployment
"""
import os
import zipfile
...
def create_zip():
    # Remove old ZIP
    zip_path = Path("GG10.zip")
    if zip_path.exists():
        zip_path.unlink()
    
    # Create ZIP with Unix paths
    with zipfile.ZipFile("GG10.zip", "w", zipfile.ZIP_DEFLATED) as zipf:
        gg10_path = Path("GG10")
        
        # Add all files from GG10 folder
        for file_path in gg10_path.rglob("*"):
            if file_path.is_file():
                # Get relative path
                relative_path = file_path.relative_to(gg10_path)
    ...
    print(f"\nGG10.zip created successfully!")
    print(f"Size: {zip_size:.2f} KB")
    
    # Verify critical files
    with zipfile.ZipFile("GG10.zip", "r") as zipf:
        critical_files = [
