# PowerShell script to automatically build GG8.zip for Netlify deployment
# This ensures the game is always ready for online testing

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Building GG8.zip for Netlify..." -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan

# Step 1: Update GG8 folder with latest files
Write-Host "`n[1/3] Updating GG8 folder..." -ForegroundColor Yellow

if (-not (Test-Path "GG8")) {
    New-Item -ItemType Directory -Path "GG8" | Out-Null
    Write-Host "Created GG8 folder" -ForegroundColor Gray
}

# Copy all necessary files
Write-Host "Copying source files..." -ForegroundColor Gray
Copy-Item -Recurse -Path "src" -Destination "GG8\src" -Force | Out-Null
Copy-Item -Path "package.json" -Destination "GG8\package.json" -Force | Out-Null
Copy-Item -Path "tsconfig.json" -Destination "GG8\tsconfig.json" -Force | Out-Null
Copy-Item -Path "vite.config.ts" -Destination "GG8\vite.config.ts" -Force | Out-Null
Copy-Item -Path "netlify.toml" -Destination "GG8\netlify.toml" -Force | Out-Null
Copy-Item -Path "index.html" -Destination "GG8\index.html" -Force | Out-Null

# Copy .env file if it exists (for Supabase configuration)
if (Test-Path ".env") {
    Copy-Item -Path ".env" -Destination "GG8\.env" -Force | Out-Null
...
if (Test-Path "supabase-setup.sql") {
    Copy-Item -Path "supabase-setup.sql" -Destination "GG8\supabase-setup.sql" -Force | Out-Null
    Write-Host "Copied supabase-setup.sql" -ForegroundColor Gray
}
if (Test-Path "supabase-matchmaking-function.sql") {
    Copy-Item -Path "supabase-matchmaking-function.sql" -Destination "GG8\supabase-matchmaking-function.sql" -Force | Out-Null
    Write-Host "Copied supabase-matchmaking-function.sql" -ForegroundColor Gray
}

$fileCount = (Get-ChildItem "GG8" -Recurse -File).Count
Write-Host "[OK] GG8 folder updated ($fileCount files)" -ForegroundColor Green

# Step 2: Create ZIP with Unix paths using Python
Write-Host "`n[2/3] Creating GG8.zip with Unix paths..." -ForegroundColor Yellow

if (Test-Path "GG8.zip") {
    Remove-Item -Force "GG8.zip"
}

# Use Python to create ZIP with correct path separators
python create-GG8-zip-unix.py 2>&1 | Out-Null

if (Test-Path "GG8.zip") {
    $zipSize = (Get-Item "GG8.zip").Length / 1KB
    Write-Host "[OK] GG8.zip created ($([math]::Round($zipSize, 2)) KB)" -ForegroundColor Green
} else {
    Write-Host "[ERROR] Failed to create GG8.zip" -ForegroundColor Red
    exit 1
}
...
$zip = [System.IO.Compression.ZipFile]::OpenRead("$PWD\GG8.zip")
$critical = @(
...
if ($allOk) {
    ...
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "GG8.zip is ready for Netlify deployment!" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Next steps:" -ForegroundColor Cyan
    Write-Host "1. Go to: https://app.netlify.com" -ForegroundColor White
    Write-Host "2. Click 'Deploy manually'" -ForegroundColor White
    Write-Host "3. Upload: GG8.zip" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "What's new in GG8:" -ForegroundColor Cyan
