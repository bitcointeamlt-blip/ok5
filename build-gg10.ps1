# PowerShell script to automatically build GG10.zip for Netlify deployment
# This ensures the game is always ready for online testing

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Building GG10.zip for Netlify..." -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan

# Step 1: Update GG10 folder with latest files
Write-Host "`n[1/3] Updating GG10 folder..." -ForegroundColor Yellow

if (-not (Test-Path "GG10")) {
    New-Item -ItemType Directory -Path "GG10" | Out-Null
    Write-Host "Created GG10 folder" -ForegroundColor Gray
}

# Copy all necessary files
Write-Host "Copying source files..." -ForegroundColor Gray
Copy-Item -Recurse -Path "src" -Destination "GG10\src" -Force | Out-Null
Copy-Item -Path "package.json" -Destination "GG10\package.json" -Force | Out-Null
Copy-Item -Path "tsconfig.json" -Destination "GG10\tsconfig.json" -Force | Out-Null
Copy-Item -Path "vite.config.ts" -Destination "GG10\vite.config.ts" -Force | Out-Null
Copy-Item -Path "netlify.toml" -Destination "GG10\netlify.toml" -Force | Out-Null
Copy-Item -Path "index.html" -Destination "GG10\index.html" -Force | Out-Null

# Copy .env file if it exists (for Supabase configuration)
if (Test-Path ".env") {
    Copy-Item -Path ".env" -Destination "GG10\.env" -Force | Out-Null
...
if (Test-Path "supabase-setup.sql") {
    Copy-Item -Path "supabase-setup.sql" -Destination "GG10\supabase-setup.sql" -Force | Out-Null
    Write-Host "Copied supabase-setup.sql" -ForegroundColor Gray
}
if (Test-Path "supabase-matchmaking-function.sql") {
    Copy-Item -Path "supabase-matchmaking-function.sql" -Destination "GG10\supabase-matchmaking-function.sql" -Force | Out-Null
    Write-Host "Copied supabase-matchmaking-function.sql" -ForegroundColor Gray
}

$fileCount = (Get-ChildItem "GG10" -Recurse -File).Count
Write-Host "[OK] GG10 folder updated ($fileCount files)" -ForegroundColor Green

# Step 2: Create ZIP with Unix paths using Python
Write-Host "`n[2/3] Creating GG10.zip with Unix paths..." -ForegroundColor Yellow

if (Test-Path "GG10.zip") {
    Remove-Item -Force "GG10.zip"
}

# Use Python to create ZIP with correct path separators
python create-GG10-zip-unix.py 2>&1 | Out-Null

if (Test-Path "GG10.zip") {
    $zipSize = (Get-Item "GG10.zip").Length / 1KB
    Write-Host "[OK] GG10.zip created ($([math]::Round($zipSize, 2)) KB)" -ForegroundColor Green
} else {
    Write-Host "[ERROR] Failed to create GG10.zip" -ForegroundColor Red
    exit 1
}
...
$zip = [System.IO.Compression.ZipFile]::OpenRead("$PWD\GG10.zip")
$critical = @(
...
if ($allOk) {
    ...
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "GG10.zip is ready for Netlify deployment!" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Next steps:" -ForegroundColor Cyan
    Write-Host "1. Go to: https://app.netlify.com" -ForegroundColor White
    Write-Host "2. Click 'Deploy manually'" -ForegroundColor White
    Write-Host "3. Upload: GG10.zip" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "What's new in GG10:" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "IMPORTANT: Update Supabase matches table to add p1Ready and p2Ready columns!" -ForegroundColor Red
    Write-Host "           Run this SQL in Supabase SQL Editor:" -ForegroundColor Red
    Write-Host "           ALTER TABLE matches ADD COLUMN IF NOT EXISTS p1Ready BOOLEAN DEFAULT FALSE;" -ForegroundColor Yellow
    Write-Host "           ALTER TABLE matches ADD COLUMN IF NOT EXISTS p2Ready BOOLEAN DEFAULT FALSE;" -ForegroundColor Yellow
    Write-Host "           ALTER TABLE matches DROP CONSTRAINT IF EXISTS matches_state_check;" -ForegroundColor Yellow
    Write-Host "           ALTER TABLE matches ADD CONSTRAINT matches_state_check CHECK (state IN ('waiting', 'ready', 'active', 'done'));" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "========================================" -ForegroundColor Cyan
} else {
    Write-Host ""
