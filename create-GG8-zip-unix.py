#!/usr/bin/env python3
"""
Create GG8.zip with Unix path separators for Netlify deployment
"""
import os
import zipfile
...
def create_zip():
    # Remove old ZIP
    zip_path = Path("GG8.zip")
    if zip_path.exists():
        zip_path.unlink()
    
    # Create ZIP with Unix paths
    with zipfile.ZipFile("GG8.zip", "w", zipfile.ZIP_DEFLATED) as zipf:
        gg8_path = Path("GG8")
        
        # Add all files from GG8 folder
        for file_path in gg8_path.rglob("*"):
            if file_path.is_file():
                # Get relative path
                relative_path = file_path.relative_to(gg8_path)
    ...
    print(f"\nGG8.zip created successfully!")
    print(f"Size: {zip_size:.2f} KB")
    
    # Verify critical files
    with zipfile.ZipFile("GG8.zip", "r") as zipf:
        critical_files = [
